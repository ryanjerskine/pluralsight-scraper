...
void (async function() {
	...
	if (!numberOfVideos) {
		console.error(
			'Failed to find videos. Check your login credentials and make sure the course exists.'
		);
		process.exit(1);
	}
	console.log(
		'Found ' +
			course.title +
			' on Pluralsight. Number of videos: ' +
		...
		if (
			fs.existsSync(
				'videos/' +
					...
					number +
					'. ' +
					course.videos[i].name.replace('/', '') +
					'.mp4'
			)
		) {
			console.log(
				'Skipping download of course video: ' +
					course.videos[i].name +
